...
// { Edge = true ; }
  ...
  reset_cookie = false ; // true = force new cookie for dev/debug

  var version = "0.7" ; 
  var script_version = 'WiViVi version ' + version ;

  var data_read = '' ;

  //set initial map coloring to show 
  var cvw = window.innerWidth;  // canvas width
  var cvh = window.innerHeight; // canvas height
  var canvas_size = 'Canvas: ' + cvw + 'x' + cvh ;

  console.log ('cvw: ' + cvw + ' cvh: ' + cvh) ;  
  document.title = "WiViVi - Wikipedia Views Visualized (ver " + version + ")"
//document.title = cvw + ' x ' + cvh ;  // dev/debug only
...
//iOS = true  ; // for tests on desktop
  ...
  var image_for_splash          = commons + "1/1d/WiViVi_pageviews_per_capita_countries_only.png" ;
  show_legend = false ; 
  if (cvh > 600)
  { show_legend = true ; } 
  ...
  var text_about_languages = '<br>Languages ranked by monthly pageviews. Only languages shown with at least 0.1% of traffic in at least one country.' ;
  ...
  if (cvw < 1150)
  { text_title_regions   = ucfirst (text_requests) + "  to any Wikipedia" ; }
  if (cvw < 950)
  { text_title_regions   = ucfirst (text_requests2) + " to any Wikipedia" ; }
  if (cvw < 750)
  { text_title_regions   = ucfirst (text_requests2) + " to any Wp" ; }
  if (cvw < 350)
  { text_title_regions   = 'to any  Wp' ; }

  var text_title_languages = "Wikipedia " + text_requests + ", percentage to language " ;
  if (cvw < 1150)
  { text_title_languages = "Wikipedia " + text_requests2 + ", % to language " ; }
  if (cvw < 1000)
  ...
  var text_switch_mode_to_languages = "<table><tr><td><img src='WorldSmall1.jpg' border=0 style='margin:0px;padding:0px;border:0px'></td><td>Show pageviews<br>for language X</td></tr></table>" ; 
  var text_switch_mode_to_regions   = "<table><tr><td><img src='WorldSmall2.jpg' border=0 style='margin:0px;padding:0px;border:0px'></td><td>Show pageviews<br>to any Wikipedia</td></tr></table>" ; // region" ;
  ...
  var max_table_rows = 2 + Math.round (cvh/39) ; // make max rows shown dependant on window height
...
//  { throw new Error('This is not an error. This is just to abort javascript'); };
  ...
  header_languages +=
    "<tr class=bubbles>" +
     ...
     "<td class=l colspan=2 style='vertical-align:middle;'><b>contribution per country</b><br><font color='#888'>(%'s add up to ~100%)</font></td>"+ 
     "</tr>";
  ...
  header_countries =
    ...
    "<tr class=bubbles>" +
     ...
     "<td class=c colspan=2 style='background-color:#DDD; vertical-align:middle;'>% of country\'s pageviews<br>to language ..." + 
                                                                 "<br><font color='#888'>(%'s add up to ~100%)</font></td>" +
     "</tr>";
  ...
  if (! (iOS || isAndroid) && (cvw > 900))
  {
    // reset cookie? for dev tests
    if (reset_cookie) 
    { setCookie (splash_screen, '') ; }
    // document.cookie = "username=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;"; 

    var show_splash_screen = getCookie(splash_screen) ;
    console.log ('cookie, splashscreen: ' + show_splash_screen) ; 

    if (show_splash_screen.indexOf ('keep') > 0)
    { 
      console.log ('keep splash screen') ;
      document.getElementById (input_modal).checked = false ;
      showModal () ; 
    }
    else if (show_splash_screen.indexOf ('skip') < 0)
    { 
      console.log ('show splash screen') ;
      setCookie (splash_screen, 'skip') ;
      showModal () ; 
    }
  }
...
//console.log (document.getElementById (btn_switch_mode)) ; // .setAttribute("class", "iOSdevice") ;
  ...
  function showMode () 
  {
    data_read = 'Data read: ' + files_read.join (', ') ;
    ...
    { 
      ...
      showStatusInFooter ('<br>' + script_version + '<br>' + data_read + '.' + sp + canvas_size) ; 
    ...
    { 
      ...
      d3.select(".infobox")      .style("display", "block") ;	
      setHtmlById (icons_explanation, icons_explanation_msg) ;	
      showStatusInFooter ('<br>' + script_version + '<br>' + data_read + '.' + sp + canvas_size + text_about_languages); 
...
//console.log (document.getElementById (btn_switch_mode)) ; // .setAttribute("class", "iOSdevice") ;
  ...
  function showStatusInFooter (status)
  {
    viz_status = status ;
    ...
    status = "<font color=#888888>" + status + "</font>" ;

    if (cvw > 1900)
    { setHtmlById (status_bottom_right, status) ; }	
    else
    { 
    //setHtmlById (status_bottom_left, status.replace (/\<br\>/g,'')) ; 
      setHtmlById (status_bottom_left, status) ;
    }	
  }
  ...
  console.log ('adaptToSmallScreen') ;
   ...
   // document.getElementById (select_box_languages).style.left = 200 ; // (cvw - 130) + 'px' ;
...
//**********************************************************************************************
  ...
  //list_select_box_regions +=  " <option value='XX' selected='selected'>none</option>\n" ; // why this ? I forgot 
    ...
    //if (':EU:AF:NA:CA:SA:AS:OC:'.indexOf (':' + data_regions [i].label + ':') == -1)  { continue ; } // do not use these after all
      ...
      if (iOS) // make radius large enough for touch 
      {
        if (cvw >  900) { radius_bubbles = 1 ; }
        if (cvw > 1200) { radius_bubbles = 12 ; }
        if (cvw > 1800) { radius_bubbles = 15 ; }
      }
      else
      {
        if (cvw >  900) { radius_bubbles =  7 ; }
        if (cvw > 1200) { radius_bubbles = 10 ; }
        if (cvw > 1800) { radius_bubbles = 12 ; }
      }
...
console.log (icon_people) ;
          metric_population = '' ;
          if (cvw > 700)
          { icon_people + sp + data.population ; } 
          ...
          info = "<div class='hoverinfo' align=left " + 
                 ...
                 metric_population + sp2 + '(' + data.perc_connected + sp + icon_connected + ')' +
...
//**********************************************************************************************
  ...
  //remove previous legend
    ...
    */
    ...
    worldmap.mylegend  ({legendTitle:"views per month<br>per capita<br>to any Wikipedia&nbsp;"}) ;
  }
...
// see https://ourworldindata.org/wp-content/uploads/datamaps/readFromCSV_autobucket_switchByButtons_infantmortality_longrun/readFromCSV_autobucket_switchByButtons_infantmortality_longrun.html
  ...
  function addCustomLegend (layer, data, options)  
  {
    ...
    if (( data.legendTitle ) && (cvh > 820)) // on small window this text interferes with hovering, no hover even on countries in the south 
    {  title = "<b>" + data.legendTitle + "</b><br>" ; } // legend title somewhat redundant, as page title tell similar story
    
    for ( var fillKey in this.options.fills ) 
    {
      console.log ('fillKey: ' + fillKey) ;
    ...
    html2 = html2.replace (/<font.*?>/g,'').replace (/<\/font>/g,'') + '<br>&nbsp;' ;

    if (cvh < 850) // show legend in one row
    { 
      console.log ('cvh: ' + cvh + ' -> html2: ' + html2) ;
      setHtmlById (legend, html2) ; 
    }
    else // show legend in block
    {
      console.log ('cvh: ' + cvh + ' -> html: ' + html) ;
...
//**********************************************************************************************
  ...
  function prepHoverBoxCountries (geo,csv_countries)
  {
    ...
    {
      showStatusInFooter ('Not all data read yet. Data read ' + files_read.join (', ')) ;  
      return ('') ;
    }
...
//  colorMapViewsPerPerson (data.viewfreq_per_country) ;
    ...
    var dialog_width = '640px' ;
    if (show_icons)
    { dialog_width = '640px' ; } // change to 420px when icons can indeed be shown again
    if ((region == 'CA') || (region == 'OC')) // extra long country names
    { dialog_width = '660px' ; } // change to 420px when icons can indeed be shown again
...
// alert doesn't work (by default) in some browsers e.g. Edge, so use status lines in window
function showStatus ()
{
  var text = viz_status ;
...
//  document.getElementById('id01').style.visibility='visible' ;
  ...
  x = document.getElementById('id01').style.display ;
  x = document.getElementById('id01') ;
console.log ('x: ' + x) ;
//  document.getElementById('id01').style.display='compact' ;
}
...
function setCookie(name, value, expires, path, domain, secure)
{
  ...
  console.log ("setCookie: '" + curCookie + "'") ;
...
function getCookie (name)
{
  ...
  {
    console.log ('getCookie: no cookie found') ;
    return nullstring;
  }
