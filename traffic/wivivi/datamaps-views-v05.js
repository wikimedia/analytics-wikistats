...
//iOS = true  ; // for tests on desktop
  ...
  if (w < 1100)
  { text_title_regions   = ucfirst (text_requests) + "  pp to any Wikipedia" ; }
  if (w < 950)
  { text_title_regions   = ucfirst (text_requests2) + " pp to any Wikipedia" ; }
  if (w < 880)
  { text_title_regions   = text_requests2 + " pp to any Wp, " + data_month ; }
  if (w < 550)
  { text_title_regions   = text_requests2 + " pp to any Wp" ; }
  if (w < 400)
  { text_title_regions   = text_requests2 + " pp" ; }
...
//console.log (document.getElementById (btn_switch_mode)) ; // .setAttribute("class", "iOSdevice") ;
  ...
  function showMode () 
  {
    ...
    { 
      ...
      showStatusInFooter ('<br>' + script_version + '<br>' + data_read + canvas_size) ; 
    ...
    { 
      ...
      d3.select(".infobox")      .style("display", "block") ;	
      setHtmlById (icons_explanation, icons_explanation_msg) ;	
      showStatusInFooter ('<br>' + script_version + '<br>' + data_read + canvas_size + text_about_languages); 
...
//**********************************************************************************************
  ...
  //list_select_box_regions +=  " <option value='XX' selected='selected'>none</option>\n" ; // why this ? I forgot 
    ...
    //if (':EU:AF:NA:CA:SA:AS:OC:'.indexOf (':' + data_regions [i].label + ':') == -1)  { continue ; } // do not use these after all
      ...
      {
        if (w >  900) { radius_bubbles = 1 ; }
        if (w > 1200) { radius_bubbles = 12 ; }
        if (w > 1800) { radius_bubbles = 15 ; }
      }
      else
      {
