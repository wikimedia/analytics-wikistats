...
sub WriteCsvGeoInfo
{
  ...
  print CSV_GEOINFO "# generated by TrafficAnalysisGeo.pl to collect all codes for countries/regions into one file, to be maintained manually\n" ;   
  ...
  print CSV_GEOINFO "# from ../csv/meta/CountryInfo.csv: population, connected, wp/en article title, icon (merged by country name) (data originally from wp/en)\n";
  print CSV_GEOINFO "# C:country info, R:region info, L:language info, F: flag info\n" ;
  print CSV_GEOINFO "record type,ISO_3166-1 alpha-2,ISO_3166-1 alpha-3,region,N/S,country name,population,connected,wp/en article title,icon file,icon width,icon height\n" ;
  ...
  foreach $iso2 (sort keys %country_names)  
  {
    next if $iso2 eq 'A1' or # MaxMind 'Anonymous Proxy'
            ...
            $iso2 eq 'EU' ;  # MaxMind 'Europe Unspecified'  
...
sub ReadCsvGeoInfo
{
  ...
  while ($line = <CSV_GEOINFO>)
  {
    ...
    {
      ($rectype,$iso2,$iso3,$region_code,$north_south_code,$country_name,$population,$connected,$article_title,$icon,$width,$height) = split (',', $line) ;
