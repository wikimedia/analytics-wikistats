...
# for ($day = 1 ; $day <= 3 ; $day++)
  for ($day = 1 ; $day <= days_in_month ($year, $month) ; $day++)
  {
    ...
    # http://code.flickr.com/blog/2009/03/18/building-fast-client-side-searches/
      ...
      # reset these arrays on new day -> animation will only show data for last day in month
      if ($bot eq 'B')
      { push @data_edits_bots,   "$secs,$lang,$country_code,$longitude2,$latitude2;" ; $lines_bot++ ; }
      else  # data for animation
...
# for ($day = 1 ; $day <= 3 ; $day++)
  for ($day = 1 ; $day <= days_in_month ($year, $month) ; $day++)
  {
    ...
    # last if $lines_read > 1000 ;
      ...
      $views_per_platform_per_location  {"$mobile,$longitude,$latitude"} ++ ;
    }
    close ($gz_in) ;
  }
...
# shrink file significantly further, to be patched back in browser
  ...
  {
    $desktop = (0 + $views_per_platform_per_location  {"-,$location"}) ;
    $mobile  = (0 + $views_per_platform_per_location  {"M,$location"}) ;
