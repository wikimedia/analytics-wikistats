...
sub ProcessLine
{
  ...
  if ($scan_ip_frequencies) # phase 1
  {
    ...
    # CHANGE: Commented this out because it was preventing from reaching the control flow for
    # classifying $mobile
...
# elsif ($agent2 =~ /Windows/io)        { ($os = $agent2) =~ s/^.*?(Windows.{10,10}[^;\(\)\[\]]*).*$/$1/io ; }
  ...
  if ($version =~ /(?:iPod|iPhone)/io)
  {
    ...
    {
      #print "\n";
      #print "======= START OF USER AGENT ======\n";
      #print "User agent string: ".$agent2."\n";
      #print "Version: ".$version."\n";
      #print "OS: ".$os."\n";
      #print "Browser engine: ".$browserengine."\n";
      #print "======= END OF USER AGENT ======\n";
      $os = "iPhone OS 1_X" ;
     }
