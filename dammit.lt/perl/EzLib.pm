...
if ($proxy ne '')
{ print "Use proxy address '$proxy'\n\n" ; }
else
{ print "No proxy defined in environment var 'https_proxy' !!\n\n" ; }
...
$trace_on_exit         = $false ; # shorthand for $trace_on_exit_concise + $trace_on_exit_libs
$trace_on_exit_concise = $false ; # show arguments and some environment info
$trace_on_exit_libs    = $false ; # show paths of libs used
...
# { use Win32 ; }
  ...
  if ($trace_on_exit || $trace_on_exit_concise)
  {
  ...
  print "trace_on_exit is set >> \n\n"         if $trace_on_exit ;
  print "trace_on_exit_concise is set >> \n\n" if $trace_on_exit_concise ;
  print "trace_on_exit_libs is set >>\n\n"    if $trace_on_exit_libs ;
  ...
  if ($trace_on_exit || $trace_on_exit_libs)
  {
    # Get library paths
    print "\nLibs:\n", map {"      $_\n"} @INC ;
