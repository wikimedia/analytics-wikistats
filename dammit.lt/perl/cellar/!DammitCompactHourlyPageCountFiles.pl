...
# removed WriteTotalsPerNameSpace, these dayta were discarded anyway on next step: merging daily aggregates into monthly aggregate
  ...
  $on_server = -d "/a/dammit.lt" ;
  $test_on_windows = ! $on_server ;

  $pagecounts = 'pageviews' ; # file changed from pagecounts-... to pageviews-... in webstatscollector 3.0 
...
# -i "D:/\@Wikimedia/!Perl/#Projects/Visitors Compact Log Files/in" -o "D:/\@Wikimedia/!Perl/#Projects/Visitors Compact Log Files/out"
  ...
  print "\nCompress pageview files\nin:  $dir_in\nout: $dir_out\nflt: $dir_filtered\ntrack: $dir_track\ndate range: $daterange" ;
  $daterange =~ s/\*/\\d+/ ;

  open LOG, ">>", "$work/WikiStatsCompactDammitFiles.log" ;

  &CompactPageviewStats ($dir_in, $dir_out, $dir_filtered, $dir_track, $daterange) ;

  &Log ("\nReady\n") ;
  close LOG ;
  exit ;

sub CompactPageviewStats
{
  my $dir_in       = shift ;
  my $dir_out      = shift ;
