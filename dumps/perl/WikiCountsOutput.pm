...
# $html_timelines .= "<div align='right'><a id='\#$scriptcnt' name='\#$scriptcnt'>End</a></div><hr><hr>\n" ;
  &Log ("Update timeline file $file_html_timelines\n") ;
  open "FILE_TIMELINES", "<", $file_html_timelines || abort ("Temp file '$file_html_timelines' could not be opened.") ;
  ...
  {
    my $info = @timelines_info {$key} ;
  # print "$info\n" ; # qqq
    my ($firstedit, $lastedit, $ns, $md5, $user) = split (",", $info) ;
    my @md5 = split ('\|', $md5) ;
  ...
  &Log ("Timelines found " . (0+$cnt_timelines) . ", skipped " . (0+$timelines_skipped) . "\n") ;
  ...
  if ($cnt_timelines > 0)
  {
    ...
    $file_html_timelines_skipped = "TimelinesSkipped" . uc ($language) . ".htm" ;
    if ($timelines_skipped > 0)
    { $text .= "<p>$timelines_skipped <a href='$file_html_timelines_skipped'>very basic, repetitive or bot created timelines</a> not shown here." ; }
  ...
  { unlink $file_html_timelines ; }

  if ($timelines_skipped == 0)
  { unlink $file_html_timelines_skipped ; }
}
...
sub RenameTempCsvFiles
{
  &LogPhase ("RenameTempCsvFiles") ;

  if (-e $file_csv_creates)
  {
    ($file_final = $file_csv_creates) =~ s/\.tmp// ;
    print "$file_csv_creates ->\n$file_final\n\n" ;
    rename $file_csv_creates, $file_final ;
  }
  if (-e $file_csv_reverted_edits)
  {
    ($file_final = $file_csv_reverted_edits) =~ s/\.tmp// ;
    print "$file_csv_reverted_edits ->\n$file_final\n\n" ;
    rename $file_csv_reverted_edits, $file_final ;
  }

  if (-e  $file_csv_reverts_sample)
  {
    ($file_final = $file_csv_reverts_sample) =~ s/\.tmp// ;
    print "$file_csv_reverts_sample ->\n$file_final\n\n" ;
    rename $file_csv_reverts_sample, $file_final ;
  }
  if (-e $file_csv_edits_per_article2)
  {
    ($file_final = $file_csv_edits_per_article2) =~ s/\.tmp// ;
    print "$file_csv_edits_per_article2 ->\n$file_final\n\n" ;
    rename $file_csv_edits_per_article2, $file_final ;
  }
}
