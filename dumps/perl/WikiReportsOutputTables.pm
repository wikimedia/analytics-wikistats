...
# $out_html =~ s/#version#/For less columns, and more figures rounded, see <a href='TablesCurrentStatusConcise.htm'>concise version<\/a>\n<p>/ ;
  ...
  $out_html .= "\n\n<table border=1 cellspacing=0 id=table2 style='' summary='Tables and charts'>\n" ;
...
sub GenerateTablesPerWiki
{
  ...
  ($dumptype,$dumpdetails) = &ReadDumpMetaData ($wp) ;
...
#  else
  $out_html .= blank_text_after ("5/07/2012", "<h3><font color=#008000><b>This report has been regenerated after recovery from recent major data collection bug. Thank you for your patience.</b></font></h3><br>") ;
  ...
  { $out_html .= "<p>Type of dump from which metrics were extracted is unknown." ; }

  if (($mode eq 'wx') and ($wp eq 'commons'))
  { $out_html .= "<br>On Commons file and category pages (namespaces 6, 14) are also seen as countable content, and contribute to editor and article counts.<br>&nbsp;</font>" ; }

  $out_html .= "<br>See also <a href='http://www.mediawiki.org/wiki/Analytics/Metric_definitions'><u>metrics definitions</u></a><br><hr>" ;
  ...
  &GenerateColophon ($false, $false, $false, $dumpdetails) ;
  $out_html .= "\n$out_script_embedded\n</body>\n</html>" ;
...
# $MonthlyStatsWpStop  {'zzw'} = ord ($m_max) - 1 ;
  ...
  &GenerateColophon ($false, $false) ;
  $out_html .= "\n$out_script_embedded\n</body>\n</html>" ;
  $file_html = $path_out . "TablesWikimediaAllProjectsWeighted.htm" ;
...
sub TableMonthlyStatsLegend
{
  ...
  $out_html .= "<p>See also <a href='http://www.mediawiki.org/wiki/Analytics/Metric_definitions'><b>Metrics definitions</b></a><p>" ;
...
sub GenerateTableEditActivityLevels
{
  ...
  if ($call_generate_table_edit_activity_levels++ == 0)
  ...
  if ($wp =~ /^zz\*w/)
  { $out_header = blank_text_after ("31/1/2013", "<font color=#008000>". &b(ucfirst($out_new)) . " July 2012</font>: ") . "Edit activity levels of registered users, weighted activity <b><font color=red>(experimental, not an official WMF metric!)</font></b>.<br><a href='TablesWikimediaAllProjects.htm'>Switch to official metric: unweighted activity</a>" ; }
  elsif ($wp =~ /^zz\*/)
  { $out_header = blank_text_after ("31/1/2013", "<font color=#008000>". &b(ucfirst($out_new)) . " July 2012</font>: ") . "Edit activity levels of registered users." ; }
  elsif ($wp =~ /^zz/)
 ...
 if ($wp =~ /^zz/)
  {
    ...
    {
      if ($wp =~ /^zz\*w/)
      { $out_explanation .= "Weighted activity: one file upload on Commons counts as five regular article edits<p>" ; }
    # elsif ($wp =~ /^zz\*/)
    # { $out_explanation .= "Weighted activity: one file upload on Commons counts as five regular article edits (not in this report !)<p>" ; }

      $out_explanation .= "Only edits to proper articles (namespace 0) are taken into account, with a few exceptions:<br>\n" .
                          "For Wikimedia Commons file uploads and file info edits (both namespace 6) and category edits (namespace 14) also counted.<br>\n" .
                          "For Wikisource namespaces 102,104,106 also count.<p>" .
                          "<p><b>Notes on count methodology:</b><br>" .
                          "Before user activity is counted, edits per user per month are totalled for all projects/wikis. So each user is counted only once.<br>" .
                          "Before July 2012 WMF used a rough approximation for establishing total editor counts:<br>\n" .
                          "Users who contributed to several wikis could be counted twice or more (only for wikis where their monthy activity reached any given threshold)<br>" .
                          "Edits to Wikimedia Commons were ignored (on the assumption that most contributors to Commons also edited on other projects, this assumption is less and less valid)<p>\n" .
                          "In each column all editors with at least x edits are included.<br>For example, an editor with 12 edits in some month would be counted in columns 1,2,5,10.\n" .
                          "<p>&nbsp;" ;
    }
    else
    {
      if ($mode_ws)
      { $out_explanation = "Edits to proper articles (namespace 0) are taken into account, plus (for wikisource only) namespaces 102,104,106.<p>" ; }
      else
      { $out_explanation = "Articles = namespace 0 only.\n" ; }

      $out_explanation .= "<p>Notes on count methodology:<br>" .
                          "Before user activity is counted, edits per user per month are totalled for all languages. So each user is counted only once.<br>" .
                          "Before July 2012 WMF used a rough approximation for establishing total editor counts:<br>\n" .
                          "Users who contributed to several wikis could be counted twice or more (only for wikis where their monthy activity reached any given threshold)<p>" .
                          "In each column all editors with at least x edits are included.<br>For example, an editor with 12 edits in some month would be counted in columns 1,2,5,10.\n" .
                          "<p>&nbsp;" ;
    }
  }
  ...
  {
    if (($mode eq 'wx') && ($wp eq 'commons'))
    { $out_explanation = "Articles = namespaces 0 (main) 6 (files)  14 (categories) - Talk pages = namespaces 1 3 5 7 etc - Other pages = other even numbered mamespaces.\n" ; }
    else
  ...
  if ($wp =~ /^zz/)
  {
    ...
    $out_explanation .=  "<p><font color=#A0A0A0>MoM/PoM: <b>Unofficial</b> quasi-normalized metrics, meant for quick and rough monthly trend assesment:</font>\n" ;
    $out_explanation .=  "<br><font color=#A0A0A0>* = As a rough 'normalization' currently only first 28 days per month are taken into account</font>" ;
    $out_explanation .=  "<br><font color=#A0A0A0>MoM = Month over Month change</font> \n" ;
    $out_explanation .=  "<br><font color=#A0A0A0>PoM = Percentage of Maximum, (= highest value ever in that column)</font><br>\n" .
                         "<font color=#FFFFDD>PoM = </font><font color=#A0A0A0>Note how a short month can have PoM 100% even when the absolute editor count is less than the all-time maximum for that column</font>\n" ;
    if (! $all_projects)
  ...
  $out_html .= "<br><a id='editor_activity_levels' name='editor_activity_levels'>&nbsp;</a>$hr<b>$out_header</b><p>$out_explanation<p>"  ;
...
#      }
    ...
    if ($wp =~ /^zz/)
    {
       $line_html3 .=  &tde .
                       ...
                       &thcb ("<font color=#808080>5 MoM</font>") . &thcb ("<font color=#808080>100 MoM</font>") . &tde .
        ...
        for ($c = 0 ; $c <= $#thresholds ; $c++)
        {
          ...
          # $line_html .= &tdlb ($count_B) ;
            ...
            if (($n == 0) && ($u == 0) && ($thresholds [$c] =~ /^(?:5|100)$/)) # only for article namespace and registered users
            {
              ...
              $perc_MoM = '-' ;
              if (($count >= 10) && ($count_month_ago > 0))
              { $perc_MoM = sprintf ("%.1f", 100 * ($count / $count_month_ago) - 100). '%' ; }
              if ($perc_MoM =~ /-/)
            # { $linepart_html_MoM .= &tdrb ("<font color=A08080>$perc_MoM</font>") ; }
              { $linepart_html_MoM .= &tdrb ("<font color=808080>$perc_MoM</font>") ; }
              else
            # { $linepart_html_MoM .= &tdrb ("<font color=80C080>$perc_MoM</font>") ; }
              { $linepart_html_MoM .= &tdrb ("<font color=808080>$perc_MoM</font>") ; }
...
sub GenerateTableUploadActivityLevels
{
  ...
  $out_explanation = "Note: uploader activity levels differ from editor activity levels (see above) in that only namespace 6 (uploaded binary files) is involved.<br>" .
                     "For every file description page only the first revision is examined to establish original uploader and upload time." ;
  ...
  if ($dumptype eq 'edits only')
  { $line_html = &thcxb ($columns {1}+2, "Users, counting all uploads") . &tdlbrx (999,'') . &thcxb ($columns {2}+2, "Via uploadwizard") . &tdlbrx (999,'') . &thcxb ($columns {3}+3, "Bots"); }
  else
...
# for ($c = 0 ; $c <= $columns {2} - 1 ; $c++) { $line_html .= &thcb ($thresholds [$c]) ; } $line_html .=  &tdlbrx (999,'') ;
  ...
  for (my $m = $m1 ; $m >= $m2 ; $m--)
  {
    ...
    foreach $cell (@cells)
    {
      ...
      if (($dumptype eq 'edits only') && ($column_set == 2))
      { $line_html .= &tdcb ('-') ; }
      else
      { $line_html .= &tdrb ($cell) ; }
      $subcol ++ ;
...
sub GenerateTableUploadsByUploadWizard
{
  ...
  while ($line = <CSV_UPLOAD_WIZARD>)
  {
    ...
    if ($dumptype eq 'edits only')
    { $line_html = &tdrb ($date) . &tdrb ($uploads) . &tdrb ($uploads_bot)  . &tdrb ($uploads_user) . &tdcb ('-') . &tdcb ('-') ; }
    else
...
sub GenerateTableEditsPerArticle
{
  ...
  $out_html .= "<br><a id='mostedited' name='mostedited'>&nbsp;</a><hr><p><b>Most edited articles</b>" .
               "<p><font color=#800000>Table out of order. Data collection needs to be revised." .
               "<br>For some projects up to date and much more extensive database reports are published from the toolserver, e.g. for " .
               "<a href='http://en.wikipedia.org/wiki/Wikipedia:DBR'>English Wikipedia</a> and <a href='https://commons.wikimedia.org/wiki/Commons:Database_reports/Pages_with_the_most_revisions'>Wikimedia Commons</a></font>" ;
  ...
  $out_html2 = "<html><body><b>Most edited articles</b>" .
               ...
               "<p>For some projects up to date and much more extensive database reports on most edited articles are published from the toolserver, e.g. for ." .
...
# }
  ...
  $out_html .= "<br><font color=#A00000>May 2016: counts have been recalculated from May 2015 onwards: the following have been added: Wikipedia Zero traffic, traffic to mobile site for other projects than Wikipedia, traffic to 'Other Projects' beyond Commons.</font> " ;
...
# foreach $code (qw (wp)) # May 2016 add mobile data for all projects not only wp 
  foreach $code (qw (wb wk wn wp wq ws wv wo wx)) # in case of wx (wikispecial), file contains only results for commons
  {
    $path_in_views = $path_pv ;
    $path_in_views =~ s/csv_$mode/csv_$code/ ;
...
# foreach $code (qw (wp)) # May 2016 add mobile data for all projects not only wp 
  ...
  $project_names {'wx'}   = 'other projects' ;
...
# foreach $code (qw (wb wk wn wo wp wp.m wp.c wq ws wv wx))
  foreach $code (qw (wb wk wn wp wp.m wp.c wq ws wv wo wx))
  {
    if ($pageviews_non_mobile)
    {
      ...
      if ($code eq 'wx')   { $link = "<a href='$root/wikispecial/EN/TablesPageViewsMonthly.htm'>Other Projects<br>&nbsp;</a>" ; }
      $line_html .= &th("&nbsp;$link&nbsp;") ;
    }
    else
...
# foreach $topic (qw (year_trend view_rates sparklines forecast forecast2))
  ...
  # foreach $code (qw (wb wk wn wp wp.m wp.c wq ws wv wo wx))
    foreach $code (qw (wb.c wk.c wn.c wp wp.m wp.c wq.c ws.c wv.c wo.c wx.c)) # May 2016: show combined counts for all non-wp projects: mobile plus non-mobile 
    {
  ...
  for ($m = $month_hi_pageviews ; $m >= $month_lo_pageviews ; $m--)
  {
    ...
    $line_html .= "\n<script language='javascript'>\n" ;
  # foreach $code (qw (wb wk wn wp wp.m wp.c wq ws wv wo wx))
    foreach $code (qw (wb.c wk.c wn.c wp wp.m wp.c wq.c ws.c wv.c wo.c wx.c)) # May 2016: use combined counts for all non-wp projects: mobile plus non-mobile
    {
