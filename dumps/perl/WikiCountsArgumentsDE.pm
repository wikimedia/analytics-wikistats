  ...
  if ($mode eq "wp")
  {
    ...
    if (@csv > 0)
    {
      ...
      if ($edits_only && ($language =~ /^(?:id|jv|sv|sw|ar|commons)$/))
      {
        ...
        &LogT ("Overrule edits only mode for selected wikis, 2012-04: outreach programs: special request by ***********\n") ;
        &LogT ("Overrule edits only mode for selected wikis, 2014-03: WikiAfrica program: special request by ***********\n") ;
