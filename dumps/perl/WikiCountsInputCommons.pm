...
sub ReadInputXml
{
  ...
  # hard coded as this is probably the last time WLM stats are taken from full archive dump,
  # otherwise collect file names from dump status page 
  # only use this code here, not for prescan mode, does not apply to commons   
  ...
  $files_xml = 0 ;
  while ($file_in_xml_full = shift (@xml_files))
  {
    $file_in_xml_full = "$folder_xml/$file_in_xml_full" ;
    $files_xml ++ ;
    print "Process xml file $files_xml: $file_in_xml_full\n" ;
