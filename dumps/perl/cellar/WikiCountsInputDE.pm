...
sub ReadInputXml
{
  ...
  if ($dump2csv)
  { 
    print "open file_trace_edits: $file_trace_edits\n" ;
    open  "FILE_TRACE_EDITS", ">",  $file_trace_edits || abort ("Perl file '$file_trace_edits' could not be opened.") ; 
    print  FILE_TRACE_EDITS "namespace,time,title,user,[article]\n" ;
    close "FILE_TRACE_EDITS" ;
    open  "FILE_TRACE_EDITS", ">>", $file_trace_edits || abort ("Perl file '$file_trace_edits' could not be opened.") ; 

    $file_trace_edits2 = $file_trace_edits ;
    $file_trace_edits2 =~ s/\.csv/2.csv/ ;
    print "open file_trace_edits2: $file_trace_edits2\n" ;
    open  "FILE_TRACE_EDITS2", ">",  $file_trace_edits2 || abort ("Perl file '$file_trace_edits' could not be opened.") ; 
    print  FILE_TRACE_EDITS2 "namespace,time,title,user,[article]\n" ;
    close "FILE_TRACE_EDITS2" ;
    open  "FILE_TRACE_EDITS2", ">>", $file_trace_edits2 || abort ("Perl file '$file_trace_edits' could not be opened.") ; 
  }
...
# print "ReadInputXmlPage title $title_full\n" ; #qqq
...
# print "1 article $article, time $time\n" ;
  # if ($time !~ /^200405/)
  #  if ($time !~ /^200(1|2|3|4)/)
  #  {
  ...
  #    next ;
  #  } ; # qqqq
# print "2 article $article, time $time\n" ;
# print "edits found $edits_found_200405 title $title\n" if ++ $edits_found_200405 % 100 == 0 ;
    my $date = substr ($time,0,8) ;
    last if $date > $dumpdate ;

    if ($first_revision)
    {
      print TITLES "$pageid,$title\n" ; # qqq
    ...
    { 
      ...
      print FILE_TRACE_EDITS &csv($namespace).&csv($time2).&csv3($title_full).&csv3($user) . "\n" ; 
    }
...
code_complete ("ProcessRevision", $start_process_revision) if $record_time_process_revision_main ;
  ...
  if (! $edits_only_page_is_redirect && $content_page) # qqqq
...
my $start_process_revision2 = code_started() if $record_time_process_revision_main ;
  ...
  {
    ($revision_is_redirect, $article_type) = &CollectArticleCounts ($title,$time,$current_revision, $namespace, $pageid, $article, $user, $userseqno) ;
...
sub CollectArticleCounts
{
  ...
  my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst)= gmtime ($time); #qqqq
  $year += 1900 ;  
  $mon++ ;
  $timestamp = sprintf ("%04d-%02d-%02dT%02d:%02d:%02dZ", $year, $month, $day, $hour, $min, $sec) ;
  ...
  if ($edits_only)
  {
    ...
    print FILE_TRACE_EDITS2 &csv($namespace).&csv($timestamp).&csv3($title).&csv3($user)."\n" ; #qqq
