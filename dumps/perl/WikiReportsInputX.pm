...
sub DetectWikiMedia
{
  ...
  else
  {
    ...
    if ($lines > 0)
    {
      ...
      if (($avg_length < 3) && ($languages {"en"} > 0) && ($languages {"de"} > 0))
      { $wikimedia = $true ; }
    }
  }
...
sub ReadMonthlyStats
{
  ...
  while ($line = <FILE_IN>)
  {
    ...
    # if ($wp eq "en") { next ; } # Dec 2006: skip till counts are fixed
...
if ($false)
{
  ...
  {
    $LargeWiki {$wp} = $false ;
    if ($MonthlyStatsHigh {$wp."E"} > ($MonthlyStatsHigh {"enE"} / 50))
    { $LargeWiki {$wp} = $true ; }
  }
...
# $MonthlyStatsWpStart {"zz"} = $m1 ;
  ...
  $MonthlyStatsWpStopLo2 = 999 ; # temp for mode_wp without "en"
  for ($m = $m1 ; $m <= $m2 ; $m++)
  {
    for ($f = 0 ; $f <= $fmax ; $f++)
    {
      ...
      foreach $wp (@languages)
      {
        ...
        {
          $wpndx ++ ;
          if (((! $mode_wp) && ($wpndx <= 20)) || $LargeWiki {$wp})
          {
            if ($m > $MonthlyStatsWpStop {$wp})
            {
      ...
      # if ($wikimedia && ($MonthlyStats {"en".$m.$c[$f]} == 0))
      # { $zz = 0 ; }
  ...
  $MonthlyStatsWpStart {"zzz"} = $MonthlyStatsWpStart {"de"} ;
}
