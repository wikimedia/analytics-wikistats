...
sub WriteTimelines
{
  ...
  if ($current_revision)
  {
    ...
    print                "\n$namespace,$title,$time,C,$title,$user,$md5s,$timelines\n\n" ;
  }
  else
  {
    print FILE_TIMELINES "$namespace,$title,$time,O,$title,$user,$md5s\n" ;
    print                "\n$namespace,$title,$time,O,$title,$user,$md5s\n\n" ;
  }
}
...
# routine for standalone processing of dump
sub CollectTimelinesFromDump
{
  my ($project, $language, $dump) = @_ ;
  if (-e $dump)
  {
    $timelines_dumps_found ++ ;
    print "upd  $project, $language, $dump\n" ;
  }
  else
  {
    $timelines_dumps_not_found ++ ;
    print "make $project, $language, $dump\n" ;
  }
}
