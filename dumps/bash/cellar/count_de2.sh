...
codes="de" # codes separated by spaces, e.g. "ab cd ef"
...
# reverts=-u 1         # uncomment to collect revert history only (-u from undo, -r was in use already)
  trace_edits=-c       # log all edits (namespace, timestamp, title, user, article)
...
# perl $perl/WikiCountsDE.pl $trace $reverts $trace_edits $edits_only $force $bz2 -m $project -i $dumps_public/$wiki -o $csv/csv_$project/ -l $wiki -d $date -s $php >> $log_step
  perl $perl/WikiCountsDE.pl $trace $reverts $trace_edits $edits_only $force $bz2 -m $project -i $dumps_public/$wiki -o $csv/csv_wp_de/ -l $wiki -d $date -s $php >> $log_step
