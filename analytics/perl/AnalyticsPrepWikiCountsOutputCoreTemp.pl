...
sub ReadStatisticsMonthlyForProject
{
  ...
  {
    if ($lines {$language}{$m_last} eq '')
    {
      print "1 wp $project:$language: no data for last month -> skip $project:$language\n" ;
      next ;
    }
  ...
  foreach $language (sort keys %languages)
  {
    if ($lines {$language}{$m_last} eq '')
    {
      print "2 wp $project:$language: no data for last month -> skip $project:$language\n" ;
      next ;
    }
